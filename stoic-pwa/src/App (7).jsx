...
// ─── Add Goal Screen ─────────────────────────────────────────────────────────
// Rendered at the same level as FocusScreen and main-app — completely outside
// the tab/scroll hierarchy. Own explicit height, own scroll, own footer.
...
function AddGoalScreen({ onSave, onCancel }) {
  const [form, setForm] = useState(BLANK_FORM)
  const valid = form.name.trim() && form.virtue

  const changeTimeframe = tf => setForm(p => ({ ...p, timeframe: tf, frequency: 1 }))
  const changeFrequency = d  => setForm(p => ({
    ...p, frequency: Math.min(PERIOD_MAX[p.timeframe], Math.max(1, p.frequency + d)),
  }))
  ...
  return (
    <div className="add-goal-screen">
      ...
      <header className="app-header">
        <span className="app-logo">NEW GOAL</span>
      </header>

      {/* Scrollable fields — own overflow-y:auto, explicit flex:1 */}
      <div className="add-goal-body">
      ...
      </div>{/* end add-goal-body */}

      {/* Pinned footer — outside the scroll body, always on screen */}
  ...
  return (
    <div style={{ height: '100dvh' }} data-size={settings.textSize}>
      {screen === 'focus' ? (
        ...
        // Rendered at the same level as FocusScreen — completely outside
        // the tab/scroll hierarchy. Own explicit CSS, own scroll.
        <AddGoalScreen
