...
// ─── Goals Tab ────────────────────────────────────────────────────────────────
// Form view mirrors ProgressTab exactly: plain div with class + padding.
// .tab-content's own overflow-y:auto scrolls it — no nested scroll containers,
// no height tricks, no flex gymnastics.
...
function GoalsTab({ goals, onUpdate }) {
  ...
  const handleCancel = () => { setView('list'); setForm(BLANK_FORM) }

  const changeTimeframe = tf => setForm(p => ({ ...p, timeframe: tf, frequency: 1 }))
  const changeFrequency = d  => setForm(p => ({
    ...p,
    frequency: Math.min(PERIOD_MAX[p.timeframe], Math.max(1, p.frequency + d)),
  }))

  // ── FORM VIEW — same structure as ProgressTab ──────────────────────────────
  if (view === 'add') {
